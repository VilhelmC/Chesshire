...
// Every preset, read off the page rather than named here: the set is
// generated by scripts/invent.mjs and hard-coding its titles here would mean
// this check goes stale the next time the generator runs.
const pills = await p.evaluate(() =>
	[...document.querySelectorAll('button')]
		.map((b) => b.innerText.trim())
		.filter((t) => /^#\d+ /.test(t)),
);
console.log('presets:', JSON.stringify(pills));
...
// Nothing on the page may print a bare coordinate pair as a move. Will:
// "we agreed all moves are accompanied by piece glyph."
{
	const bare = await p.evaluate(() => {
		const text = document.body.innerText;
		const m = text.match(/(^|[\s—(])[a-h][1-8][–x×-][a-h][1-8]/g);
		return m ?? [];
	});
	console.log('\nmoves without a glyph:', JSON.stringify(bare));
}

await p.getByRole('button', { name: pills[0], exact: true }).click();
