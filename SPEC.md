...
That is right and the alternating search was over-reach. The correct structure is the same fold over a larger participant list — `participants(k)` = bearing now, plus everything within `k` moves, both sides — with tempo entering as the index rather than as a search. The constant-tempo assumption that standard SEE makes is then *inherited*, and the honest thing is to enumerate how it fails rather than to search past it: the target leaves, a defender arrives sooner, or someone interposes a forcing move. Three checkable lists, which is also what a human should check. `SEE.md` §4–5 states this. **Done in M19.6** — `race.ts` is deleted and `knot.ts` answers by counting and listing.

#### M19.6 — the generator, and the six things it found

Will's closing point on the Lab's examples was the sharpest one in the thread:

> If the algorithm was working correctly then we should not have all these erroneously constructed examples, because they would be generated by inverting it.

He is right, and it is a general statement about this whole project: when the same person writes the procedure and picks the positions it is judged on, the fixture inherits the error and the tests pass. Two scripts now close that loop.

**`scripts/agree.mjs`** generates random positions, runs the algorithm on a square, and adjudicates with Stockfish. Its first run printed a verdict of *"take it now: 500"* about a square White had nothing bearing on — the 500 was a rook hanging on the other side of the board, found by `race.ts` and printed under the wrong heading. That is the defect that made every hand-built fixture come out misconstructed: I was inverting a function that answers a different question from the one the table asks.

`domain/race.ts` is deleted. `domain/knot.ts` replaces it with SEE.md §4's structure — `participants(k)`, the same fold over the longer list, and the three spoiler lists — and it makes claims only at k = 0 and k = 1, where one move is one move for both sides. From k = 2 the table says *"not a claim — see below"* rather than a number.

Five more defects came out of the same script, none of which would have occurred to me:

- **The fold captured the king.** `VALUE.king` is 100000 and nothing enforced the comment next to it, so the Lab printed exchanges worth "100100" — not a rounding error but a sequence played past the end of the game. The chain now ends when a king captures, and a king may not capture into a square the other side still guards.
- **A tempo was billed for every piece that could have arrived**, not the one that moved. In one preset that was 320 centipawns of imaginary travel charged to a pawn push, and it read as "nothing to win" for a knight that was winnable.
- **A route that grabbed a pawn on the way had a negative cost, and funded the claim.** Clamped at zero. A preparing move that also wins something is a good move; it is not evidence about this square.
- **The recapture was counted twice** — once inside the capture's own resolved price and again as a "counter-threat".
- **A threat that existed anyway was charged to the capture.** A bishop already hanging elsewhere is lost whether you take here or not. Every quantity is now a difference from doing nothing.

Measured agreement after the six fixes, on 60 generated positions with already-decided ones excluded: **58/60**, and both remaining disagreements are the referee's limit — the engine had something bigger on offer, so its line never visits the square.

**The referee had the same disease.** It asked "is White better here", which counts a rook won two moves earlier as agreement about this square; and `test/adjudicate.test.ts` was comparing whole-position evaluations to per-square claims for the same reason. Both now walk the engine's own principal variation to the first capture on the target and measure only what changes hands there.

**The examples are generated now.** `scripts/invent.mjs` runs the algorithm over random positions and keeps the first that lands in each category — the categories being the algorithm's own outputs, so the set is a picture of what the procedure can say rather than a gallery of what I hoped it would say. Six of the seven were found this way and each is engine-adjudicated. Two results worth keeping:

- **A preparing move whose knot survives every legal reply is rare** — the generator could not produce one in 2,500 positions, so the Lab's #2 is still hand-built and labelled `origin: 'hand-built'`. Stockfish plays its move.
- **One candidate was refused rather than shipped.** The generator offered a position for "it cannot run — something stands behind it" whose own headline was "take it now": the piece was loose as well as loaded, so the example would have demonstrated a mechanism its title never mentioned. Filling the slot anyway would have been this thread's mistake in a new costume.

**And the commentary is generated from the computation.** Will: *"We need correct commentary to be generated analytically from the algorithm results rather than hardcoded"*, and *"we agreed all moves are accompanied by piece glyph"*. No `expect` string is printed as an explanation any more. The verdict is assembled from the knot's own numbers — the move it proposes, the trade read off the fold's steps, the best defensive try and the capture that answers it — and every move carries a figurine. `scripts/lab-check.mjs` now asserts that no bare coordinate pair appears anywhere on the page.

For the pinned knight the machine writes, unprompted:

> ♙e2–e4 first, and the knight on d5 is worth 2.2 pawns — a knight for a pawn. It cannot simply run: ♞d5–c3 lets ♖d1×d8 collect what it was shielding. Their best try is ♟b7–b5, and ♙e4×d5 still takes it: 2.2 pawns.

Stockfish's first move in that position is e2–e4.
